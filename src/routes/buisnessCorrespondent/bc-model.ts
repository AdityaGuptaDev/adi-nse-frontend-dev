...
export interface BcRegistrationAttributes {
  regId: number;
...
type BcRegistrationCreationAttributes = Optional<
  BcRegistrationAttributes,
  "regId"
>;

export class BcRegistration
  extends Model<BcRegistrationAttributes, BcRegistrationCreationAttributes>
  implements BcRegistrationAttributes {
  ...
  static initModel(sequelize: Sequelize): typeof BcRegistration {
    BcRegistration.init(
      {
        regId: {
      ...
      {
        sequelize,
        tableName: "bc_registrations",
        timestamps: false,
      }
    );

    return BcRegistration;
  }
}
