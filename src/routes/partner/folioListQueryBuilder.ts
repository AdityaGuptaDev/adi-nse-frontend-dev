  ...
  private kfinSelect(): string {
    return `
      SELECT 
        ...
        pin AS b_pincode,
        CASE 
          WHEN dob IS NOT NULL AND dob::text ~ '^\\d{4}-\\d{2}-\\d{2}$' THEN dob::date 
          ELSE NULL 
        END AS inv_dob,
        mobile AS mobile_no,
        occpn AS occupation,
        ...
        brokcode,
        CASE 
          WHEN crdate IS NOT NULL AND crdate::text ~ '^\\d{4}-\\d{2}-\\d{2}$' THEN crdate::date 
          ELSE NULL 
        END AS folio_date,
        NULL AS aadhaar,
        NULL AS tpa_linked,
        ...
        NULL AS jh2_dob,
        CASE 
          WHEN guardian32 IS NOT NULL AND guardian32::text ~ '^\\d{4}-\\d{2}-\\d{2}$' 
          AND guardian32::text != '0' AND guardian32::text != '' 
          THEN guardian32::date 
          ELSE NULL 
        END AS guardian_d,
        NULL AS amc_code,
        state AS gst_state_,
