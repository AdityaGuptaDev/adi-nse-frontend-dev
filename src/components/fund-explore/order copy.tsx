...
const OrderPopup: React.FC<InvestorPopupProps> = ({
    ...
    const handleTransact = async () => {
        ...
        try {
            const result = await executeMfuTransaction(transaction);
            console.log("Transaction result:", result);
        } catch (err: any) {
            if (err?.err) {
                console.error("Handled transaction error:", err.err);
            } else {
                console.error("Unhandled error during transaction:", err);
            }
        }

    }

    const handleTransact00 = async () => {
        ...
        if (transactionType === 'B') {
            ...
            const payload = generateTransactionByType("B", { //For Lumpsum
                entGroupRefNo: refNo?.data?.data?.reference ?? "",
                can: selectedCan?.value ?? investor?.can_id,
                arnCode: "ARN-104974",
                totAmt: amount,
                schList: [
                    {
        ...
        } else if (transactionType === 'R') { //For Redeemption
            const payload = generateTransactionByType("R", {
                entGroupRefNo: refNo?.data?.data?.reference ?? "",
                can: selectedCan?.value ?? investor?.can_id,
                arnCode: "ARN-104974",
                totAmt: amount,
        ...
        } else if (transactionType === 'O') { //For switch Transaction
            const payload = generateTransactionByType("O", {
                entGroupRefNo: refNo?.data?.data?.reference ?? "",
                can: selectedCan?.value ?? investor?.can_id,
                arnCode: "ARN-104974",
                totAmt: amount,
        ...
        } else if (transactionType === 'Y') { //For STP
            const payload = generateTransactionByType("E", {
                entGroupRefNo: refNo?.data?.data?.reference ?? "",
                can: selectedCan?.value ?? investor?.can_id,
                arnCode: "ARN-104974",
                totAmt: "",
            ...
            console.log(JSON.stringify(payload, null, 2));
            const response = await ApiFinTechSystematicTxnService(payload);
            console.log(response)
            const result = JSON.parse(response?.data?.data);
            console.log(result)
            const appLink = result?.respBody?.ordDtl?.appLinkPri;
        ...
        } else if (transactionType === 'J') { //For SWP
            const payload = generateTransactionByType("J", {
                entGroupRefNo: refNo?.data?.data?.reference ?? "",
                can: selectedCan?.value ?? investor?.can_id,
                arnCode: "ARN-104974",
                totAmt: "",
        ...
        } else if (transactionType === 'V') { //For SIP
            const payload = generateTransactionByType("V", {
                entGroupRefNo: refNo?.data?.data?.reference ?? "",
                can: selectedCan?.value ?? investor?.can_id,
                arnCode: "ARN-104974",
                totAmt: amount,
                                    ...
                                    <table className="w-full text-xs ">
                                        ...
                                        <tbody>
                                            ...
                                            {bankList.map((opt: any) => (
                                                <tr key={opt?.mfu_can_bank_dtl_id} className="hover:bg-gray-50 border-b border-gray-100">
                                                    <td className="py-2 px-3">
                                                        <input
                                                            type="radio"
                                                            name="folio"
                                    ...
                                    {isTransact ?
                                        <>
                                            ...
                                            <div className="flex gap-3 pt-4">
                                                ...
                                                <CustomButton onClick={handleTransact}> Order Now</CustomButton>
                                                <CustomButton className="bg-gray-300 hover:bg-gray-400 text-black">Cancel</CustomButton>
                                        ...
                                        <>
                                            {isCartAdded &&
                                                <>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <CustomButton className="bg-gray-500 hover:bg-gray-600 " onClick={onClose}>Cancel</CustomButton>
