...
import { viewCanDetails, generateReference, ApiFinTechNormalTxnService, searchByAmcId, searchByISIN, searchByCanIdmfuBankDetails, ApiFinTechSystematicTxnService, submitMfuTransaction, fetchClientIp, getMandates, getInvestorPortfolio } from "@/api/transaction";
...
const OrderPopup: React.FC<InvestorPopupProps> = ({
    ...
    const handleRadio = (e: any) => {
        ...
        if (transactionType === 'R' || transactionType === 'O') {
            ...
            // setIsSelected(true);
            if (value === "Amount" || value === "Unit" || value === "All Units") {
                setPlaceHolder("Enter " + value)
                setIsSelected(true);
            } else {
                setIsSelected(true);
    ...
    const handleTransact = async () => {
        ...
        setErrors(newErrors);
        const hasError = Object.values(newErrors).some((msg) => msg !== "");
        if (paymentMode.length == 0 && transactionType != 'O' && transactionType != 'S') {
        ...
        if (!hasError) {
            ...
            if (transactionType == "B" || transactionType == "R" || transactionType == "O") {
                subSeqPayFlag = "";
                ...
                paySec = {
                    ...
                    //accType: accType,
                    //accNo: accNo,
                    //payDate: transactionType == 'O' ? '' : new Date().toISOString().slice(0, 10),
                    //payAmt: transactionType == 'O' ? '' : _amount?.toString(),
                    beneVan: "",
                    paymentRefNo: "",
                    paymentBankRefNo: "",
                    mandateRefNo: "",
                ...
                schList.push({
                    ...
                    folio: folioSelectionMode === "new" ? "NEW" : selectedFolio ?? "NEW",
                    //folio: "41007859/68",
                    //divOpt: transactionType === "R" ? "" : option?.code?.toString() ?? "",
                    divOpt: "",
                    txnVolTyp: "A",
                    vol: _amount ?? "",
                ...
                if (transactionType == "V") {
                    ...
                    subSeqSec = {
                        ...
                        ifsc: ifsc,
                        paymentRefNo: "",
                        mandateRefNo: selectedMandate?.prn
            ...
            const transaction: TransactionData = {
                ...
                can: selectedCan,
                totAmt: transactionType == "J" ? "" : _amount,
                schList: schList,
                paySecFlag: "",
                paySec: paySec,
    ...
    const handleSelectedTargetScheme = async (targetScheme: any) => {
        ...
        if (count > 0) {
            const records = response?.data?.data?.data.filter((item: any) => item.txn_type === 'I');
            if (records.length === 1) {
                setOutRtaSchCode(records[0]?.scheme_code)
            } else {
                toastAlert("info", "Multiple schemes found for the selected target scheme. Please contact support.");
            }
        } else if (count === 0) {
